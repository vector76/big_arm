...
| Actuation | NEMA 17 steppers (on hand: StepperOnline 17HS19-2004S1 — 59 N·cm holding, 2 A, 48 mm; pull-out ~41 N·cm flat to ~450 rpm at 24 V, ~17 N·cm at 1200 rpm) on 24 V drivers, with high-ratio reduction (mechanism TBD: belts, worm, cycloidal, cable/capstan…). Fans on the motors if thermal limits bind. |
| Joint travel | Base yaw ±90° (up to ~240° if useful); shoulder ~120°; **elbow bends downward only** (~135° travel — the wide wrist travel recovers reachability, sacrificing only some poses above the shoulder); wrist ±90°. |
